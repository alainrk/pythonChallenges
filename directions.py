...
From: https://www.codewars.com/kata/directions-reduction/python
Description:
...
def areOpposite(p, q):
   return (p == "NORTH" and q == "SOUTH") or (q == "NORTH" and p == "SOUTH") or (p == "WEST" and q == "EAST") or (q == "WEST" and p == "EAST")

def dirReduc(arr):
    c = 0
    while c <= len(arr)-2:
        print(c,arr)
        if areOpposite(arr[c], arr[c+1]):
            del(arr[c])
            del(arr[c])
            c = 0
        else:
